...
export class FeatureModelMulti {
    constructor(xml, features, constraints, root, featureDict, title, id) {
        this.xml = xml;
        this.title = title;
        this.id = id;
        this.hovered = false;
    ...
    static loadXmlDataFromFile(fileText, title, id) {
        ...
        const usedFeatures = [];
        const featureDict = {};
        const root = FeatureModelMulti.parseChildren(struct, null, usedFeatures, 0, featureDict)[0];

        const constraints = FeatureModelMulti.readConstraints(
            [...xmlDocument.querySelector('constraints').childNodes],
            featureDict
        );

        return new FeatureModelMulti(xml, usedFeatures, constraints, root, featureDict, title, id);
    }
    ...
    static loadXmlDataFromConfig(fileText) {
        ...
        const usedFeatures = [];
        FeatureModelMulti.parseChildrenFromConfig(struct, usedFeatures, 0);
    ...
    static parseChildren(struct, parent, usedFeatures, count, featureDict) {
        ...
        for (const child of struct.childNodes) {
            ...
            if (child.tagName) {
                ...
                toAppend.children = FeatureModelMulti.parseChildren(child, toAppend, usedFeatures, count, featureDict);
        ...
        return constraints
            ...
            .map((rule) => {
                return [...rule.childNodes]
                    ...
                    .map(
                        (item) => new Constraint(FeatureModelMulti.readConstraintItem(item, featureMap))
                    )[0];
            });
    }
    ...
    static readConstraintItem(item, featureMap) {
        if (item.tagName === 'var') {
            ...
            const childItems = [...item.childNodes]
                ...
                .map((childItem) => FeatureModelMulti.readConstraintItem(childItem, featureMap));
